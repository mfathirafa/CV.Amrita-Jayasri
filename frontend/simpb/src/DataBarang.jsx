...
const DataBarang = ({ onNavigate, onLogout }) => {
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Hapus data dari state agar UI terupdate otomatis tanpa refresh
        setTableData(prevData => prevData.filter(item => item.id !== itemToDelete.id));
        setIsDeleteModalOpen(false); // Tutup modal
        
        // Opsional: Tampilkan toast/notifikasi sukses di sini
      } else {
                          ...
                          <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="py-4 px-6 text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                {/* Tombol Hapus (Ubah fungsi pemanggilannya) */}
                                <button 

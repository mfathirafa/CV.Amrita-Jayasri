...
import { X, Building2, MapPin, Phone, Info, Loader2 } from 'lucide-react'; // <-- Tambahkan Loader2
...
const EditKonsumenModal = ({ isOpen, onClose, onSave, konsumenData }) => {
  ...
  // Mengisi form dengan data konsumen dari database saat modal dibuka
  useEffect(() => {
    if (konsumenData && isOpen) {
      setFormData({
        // Menyesuaikan dengan nama field dari database yang dilempar oleh parent
  ...
  const handleSubmit = async () => {
    ...
    // Format payload sesuai dokumentasi JSON
    const payload = {
    ...
    try {
      ...
      // Arahkan ke endpoint PUT /konsumen/{id}
      ...
      if (response.ok && data.success) {
        // Berhasil simpan, panggil fungsi onSave() dari parent untuk mereload tabel

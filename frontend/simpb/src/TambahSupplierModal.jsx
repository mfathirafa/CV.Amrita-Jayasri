...
import CancelConfirmModal from './CancelConfirmModal'; // <-- Import modal konfirmasinya
...
const TambahSupplierModal = ({ isOpen, onClose, onSave }) => {
  ...
  // Fungsi untuk mengecek sebelum menutup modal
  const handleRequestClose = () => {
    // Jika user sudah mengetik sesuatu, tampilkan pop-up konfirmasi
    if (formData.name !== '' || formData.address !== '' || formData.phone !== '') {
      setShowCancelConfirm(true);
    } else {
      // Jika masih kosong, langsung tutup saja
      onClose();
    }
  };

  // Fungsi jika user benar-benar yakin ingin membatalkan
  const handleConfirmCancel = () => {
    setShowCancelConfirm(false); // Tutup pop-up konfirmasi
    setFormData({ name: '', address: '', phone: '' }); // Reset data form
    ...
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white rounded-[24px] w-full max-w-[480px] p-8 shadow-2xl relative animate-in zoom-in-95">
          ...
          {/* Tombol Close (Ubah onClick memanggil handleRequestClose) */}
          <button 
          ...
          <div className="flex gap-4 mt-8 mb-6">
            {/* Tombol Batal (Ubah onClick memanggil handleRequestClose) */}
            <button 
              onClick={handleRequestClose} 
      ...
      {/* RENDER MODAL KONFIRMASI PEMBATALAN DI SINI */}
      <CancelConfirmModal 
        isOpen={showCancelConfirm}
        onResume={() => setShowCancelConfirm(false)} // Menutup modal batal, kembali mengedit
        onConfirmCancel={handleConfirmCancel} // Setuju untuk membatalkan
      />
    </>
  );
};
